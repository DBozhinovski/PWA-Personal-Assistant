...
export const data: TrainingData = {
  ...
  "answers.name": {
    ...
    answers: [
      (data) =>
        `Good to meet you ${data?.name}! Can I help you with something?'`,
  ...
  help: {
    ...
    answers: [
      () => {
        const skillDescriptionKeys = Object.keys(data).filter(
          (k) => data[k].description
        );

        return `Things I can do for you:
        ${skillDescriptionKeys
          .map((k) => `- ${data[k].description}`)
          .join("")}`;
      },
    ],
  },
